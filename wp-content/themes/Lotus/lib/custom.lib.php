	...
	if(!empty($posts))
	{
		...
		$return_html.= '<ul class="posts blog">';
			...
			foreach($posts as $post)
			{
				...
				if(!empty($image_thumb))
				{
					$return_html.= '<a style="align:left;display:block " href="'.get_permalink($post->ID).'"><img src="'.get_bloginfo( 'stylesheet_directory' ).'/timthumb.php?src='.$image_thumb[0].'&amp;h=200&amp;w=250&amp;zc=1" alt="" class="img_nofade"/></a>';
				}
				$return_html.= '<h6 style="position:relative"class="cufon"><a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a></h6>';
				$return_html.= '<p style="position:relative"class="custom_post_content">'.pp_substr(strip_tags(strip_shortcodes($post->post_content)), 100).'</p></li>';
