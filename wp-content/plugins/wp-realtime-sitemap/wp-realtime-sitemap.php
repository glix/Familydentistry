			...
			if (preg_match('|wp-realtime-sitemap|', $code)) {
				...
				// Nav Menu: Yes/No? - Appearance -> Menus
 				if (($all_options['show_menu'] != 'No' && $all_options['show_menu'] != 'Off') && ($show == 'menu' || $show == 'all')) {
					...
					$menu = '<div id="wp-realtime-sitemap-menu" class="realtime_sitemap"><h3>' . $menu_header . '</h3>';
				...
				if (($all_options['show_pages'] != 'No' && $all_options['show_pages'] != 'Off') && ($show == 'pages' || $show == 'all')) {
					...
					$pages = '<div id="wp-realtime-sitemap-pages" class="realtime_sitemap"><h3>' . $pages_header . '</h3>';
				...
				if (($all_options['show_posts'] != 'No' && $all_options['show_posts'] != 'Off') && ($show == 'posts' || $show == 'all')) {
					...
					if ($all_options['post_show_categories'] != 'No') {
						...
						if (count($thecategories) > 0) {
							...
							$posts = '<div id="wp-realtime-sitemap-posts" class="realtime_sitemap"><h3>' . $posts_header . '</h3><ul>' . $posts . '</ul></div>';
						}
				...
				if (($all_options['show_custom_post_types'] != 'No' && $all_options['show_custom_post_types'] != 'Off') && ($show == 'custom-posts' || $show == 'all')) {
					...
					foreach ($post_types as $post_type) {
						...
						if (count($theposts) > 0) {
							...
							$custom_posts[$post_type->name] = '<div class="realtime_sitemap" id="wp-realtime-sitemap-' . strtolower($post_type->name) . '"><h3>' . $post_type->labels->name . '</h3><ul>' . $custom_post_type_posts . '</ul></div>';
						}
					}
				}
				...
				if (($all_options['show_archives'] != 'No' && $all_options['show_archives'] != 'Off') && ($show == 'archives' || $show == 'all')) {
					...
					$archives = '<div id="wp-realtime-sitemap-archives " class="realtime_sitemap"><h3>' . $archives_header . '</h3>';
				...
				if (($all_options['show_categories'] != 'No' && $all_options['show_categories'] != 'Off') && ($show == 'categories' || $show == 'all')) {
					...
					$categories = '<div id="wp-realtime-sitemap-categories" class="realtime_sitemap"><h3>' . $categories_header . '</h3>';
				...
				if (($all_options['show_tags'] != 'No' && $all_options['show_tags'] != 'Off') && ($show == 'tags' || $show == 'all')) {
					...
					$tags = '<div id="wp-realtime-sitemap-tags" class="realtime_sitemap"><h3>' . $tags_header . '</h3>';
				...
				if (($all_options['show_promote'] != 'No' && $all_options['show_promote'] != 'Off') && ($show == 'all')) {
					// $promote = '<p style="clear:both" align="center">Sitemap created with <a href="http://goo.gl/ri9xU" title="WP Realtime Sitemap">WP Realtime Sitemap</a>.</p>';
				}
